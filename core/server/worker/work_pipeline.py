...
from .audio import process_audio_work
from . import logger
...
class WorkPipeline:
    ...
    基于工作单元来源（mic/file）和引擎能力（Capabilities）自适应调整流水线深度。
    """
    ...
    def process(self, work: Work) -> Result:
        ...
        """
        try:
            logger.info(f"工作单元 {work.task_id[:8]}, 语言={work.language}, 来源={work.source}")
            is_first_segment = work.task_id not in self.state.sessions
            session = self.state.get_session(work.task_id, work.socket_id, work.source)
            result = session.result

            # 2. 预处理音频并获取采样点
            samples = process_audio_work(work, result)

            # 空音频或极短音频，跳过推理直接返回
            if samples is None:
                result.time_start, result.time_submit = work.time_start, work.time_submit
                result.time_complete = time.time()
                result.is_final = work.is_final
                return result

            # 3. 执行识别推理
            stream = self.recognizer.create_stream()
            stream.accept_waveform(work.samplerate, samples)
            self.recognizer.decode_stream(stream, context=work.context, language=work.language)
            ...
            if (work.source == 'file'
                ...
                align_res = self.aligner.align(audio=samples, text=stream.result.text, language=work.language, offset_sec=0.0)
            ...
            logger.info(f"工作单元完成: {work.task_id[:8]}, 时长={result.duration:.2f}s, 耗时={process_time:.3f}s, RTF={rtf:.3f}")
